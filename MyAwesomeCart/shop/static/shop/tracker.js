...
function main() {
    ...
    */
    const trackOrder = async (tracking_data, csrftoken) => {
        try {
    ...
    */
    function populateTrackedOrder() {
        tracking_form.addEventListener('submit', async (e) => {
            ...
            const csrftoken = tracking_form_data.get('csrfmiddlewaretoken');

            // Call trackOrder to get the tracked order data.
            order_track = await trackOrder(tracking_form_data, csrftoken);
